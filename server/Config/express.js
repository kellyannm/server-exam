...
module.exports = function (app, config) {
  ...
  app.use('/api', gadgets);
	router.route('/gadgets').get((req, res, next) => {
     ...
     res.status(201).json({message: 'Get all gadgets'});
    });
  app.use('/api', gadgets/ID);
   router.route('/gadgets').get((req, res, next) => {
    logger.log('info', 'Get all gadgets based on ID');
      res.status(201).json({message: 'Get all gadgets based on ID'});
    });
	};
  app.use('/api', gadgets);
	router.route('/gadgets').post((req, res, next) => {
		logger.log('info', 'Create Gadget');
		 res.status(201).json({message: 'Created gadget'});

  });
  app.use('/api', gadgets/ID);
	router.route('/gadgets').put((req, res, next) => {
		logger.log('info', 'Update a gadget');
		 res.status(201).json({message: 'Updated a gadget'});

  });
  app.use('/api', gadgets/ID);
	router.route('/gadgets').delete((req, res, next) => {
		logger.log('info', 'Delete a gadget');
		 res.status(201).json({message: 'Deleted a gadget'});
